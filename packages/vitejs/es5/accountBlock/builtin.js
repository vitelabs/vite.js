...
function formatAccountBlock(_a) {
    ...
    if (message) {
        var msgHex = "0002" + Buffer.from(message).toString('hex');
...
function getCreateContractData(_a) {
    var abi = _a.abi, hexCode = _a.hexCode, params = _a.params, _c = _a.confirmTimes, confirmTimes = _c === void 0 ? 0 : _c, _d = _a.times, times = _d === void 0 ? 10 : _d;
    var jsonInterface = getAbi(abi);
    var _confirmTimes = new BigNumber(confirmTimes).toArray();
    var _times = new BigNumber(times).toArray();
    var data = vitejs_constant_1.Delegate_Gid + "01" + Buffer.from(_confirmTimes).toString('hex') + Buffer.from(_times).toString('hex') + hexCode;
    if (jsonInterface) {

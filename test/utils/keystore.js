...
import { isValid, decrypt, encrypt, encryptV1ToV3, encryptOldKeystore } from 'utils/keystore';
...
if (process.env.NODE_ENV !== 'testWatch') {
    ...
    describe('Keystore', function() {
        it('valid V1', function () {
            let result = isValid(ksV1);
            assert.equal(!!result, true);
        });
        it('valid V2', function () {
            let result = isValid(ksV1toV2);
            assert.equal(!!result, true);
        });
        it('valid V3', function () {
            let result = isValid(ksV1toV3);
            assert.equal(!!result, true);
        });
        ...
        it('encrypt version 1 to version 3', function(done) {
            let k3 = encryptV1ToV3(entropyV1, ksV1);
